...
PARAMS = {
    ...
    "sensor_sigma": {
        ...
        "source": "Measured from NHSI-meat-overtime cube 01.mat (Wang et al. 2026) via "
                   ...
                   "that double-counts the since-cut texture_amplitude. "
                   "NORMALISATION -- measured, and DELIBERATELY NOT CHANGED. 0.0054 is a "
                   "RELATIVE figure (absolute sd 0.00039 / tray mean 0.072 = 0.5% of "
                   "signal), but generate_sample() line ~663 applies it as an ABSOLUTE "
                   "additive sd: cube + rng.normal(0, 0.0054). Checked against the frozen "
                   "dataset: simulated meat averages 0.364 reflectance, so the applied "
                   "noise is 1.5% of signal -- 3x the measured relative noise, and 14x the "
                   "measured absolute noise (0.0054 vs 0.00039). (An earlier note here "
                   "estimated 3-5x by assuming relative application; the code is absolute, "
                   "so 14x is the correct absolute figure.) NOT corrected, on purpose: the "
                   "mismatch makes the inverse problem HARDER, so every reported result is "
                   "conservative, and lowering a noise parameter once results are known "
                   "would be indefensible whatever it did to the numbers. Disclose it in "
                   "Ch.5 as a stated limitation. "
